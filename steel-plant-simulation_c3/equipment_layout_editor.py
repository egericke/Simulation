...
class LayoutScene(QGraphicsScene):
    ...
    def mouseReleaseEvent(self, event):
        ...
        # Handle ladle path mode clicks - forward to the ladle path editor
            ...
            # Find the ladle path editor and forward the click
            for view in self.views():
                if hasattr(view.window(), "ladle_path_editor"):
                    view.window().ladle_path_editor.add_waypoint(event.scenePos())
                    event.accept()
                    return
...
class EquipmentLayoutEditor(QDialog):
    ...
    def create_ui(self):
        ...
        # Add Ladle Path Action
        self.path_action = QAction(QIcon.fromTheme("draw-path", QIcon("icons/path.png")), "Draw Ladle Car Path", self)
        self.path_action.setCheckable(True)
        self.path_action.triggered.connect(self.toggle_ladle_path_mode)
        ...
        # Tab 1: Equipment & Routes
        equipment_tab = QWidget()
        tools_layout = QVBoxLayout(equipment_tab)
        ...
        tools_tabs.addTab(equipment_tab, "Equipment & Routes")
        
        # Tab 2: Ladle Car Paths
        from ladle_path_editor import LadlePathEditor
        self.ladle_path_editor = LadlePathEditor()
        tools_tabs.addTab(self.ladle_path_editor, "Ladle Car Paths")
        ...
        props_layout.addWidget(bg_group)
        props_layout.addStretch()
        tools_tabs.addTab(props_tab, "Properties")
        
        # Tab 4: Route Simulation
        self.route_simulator = RouteSimulator()
        self.route_simulator.set_scene(self.scene)
        tools_tabs.addTab(self.route_simulator, "Route Simulation")
        
        splitter.addWidget(tools_tabs)
        ...
        # Setup route simulator and ladle path editor
        self.route_simulator.set_scene(self.scene)
        self.ladle_path_editor.set_scene(self.scene)
    ...
    def load_layout(self):
        ...
        try:
            ...
            if "ladle_car_paths" in layout_data:
                self.config["ladle_car_paths"] = layout_data["ladle_car_paths"]
